...
describe("legacy extraction retirement", () => {
	...
	beforeEach(() => {
		...
		db.exec(`
			CREATE TABLE memories (id TEXT PRIMARY KEY, extraction_status TEXT, memory_kind TEXT, is_deleted INTEGER DEFAULT 0);
	...
	it("terminalizes every unfinished retired extraction job without creating replacement work", () => {
		...
		db.prepare("INSERT INTO memories (id, extraction_status, is_deleted) VALUES (?, ?, 1)").run(
			"deleted-memory", "queued",
		);
		db.prepare("INSERT INTO memory_jobs (id, memory_id, job_type, status, updated_at) VALUES (?, ?, ?, ?, ?)").run(
			"pending-job", "pending-memory", "extract", "pending", now,
		);
		db.prepare("INSERT INTO memory_jobs (id, memory_id, job_type, status, updated_at) VALUES (?, ?, ?, ?, ?)").run(
			"leased-job", "leased-memory", "extract", "leased", now,
		);
		db.prepare("INSERT INTO memory_jobs (id, memory_id, job_type, status, updated_at) VALUES (?, ?, ?, ?, ?)").run(
			"failed-job", "failed-memory", "extract", "failed", now,
		);
		db.prepare("INSERT INTO memory_jobs (id, memory_id, job_type, status, updated_at) VALUES (?, ?, ?, ?, ?)").run(
			"deleted-job", "deleted-memory", "extract", "pending", now,
		);

		expect(retireLegacyExtractionJobs(accessor, { reason: "Dreaming owns semantic writes" })).toBe(3);
