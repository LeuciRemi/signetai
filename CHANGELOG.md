...
- Bug fixes: extend marketplace proxy startup; cap marketplace MCP clients; preserve low-signal response shape; clarify telemetry data loss; show telemetry data loss; preserve Pi zero timeout; bound Pi concurrency; guard structural audit rollback; retire structural jobs; honor shared hint recall; scope prospective hints; bound filesystem reads by stat; async filesystem connector I/O; cap document ingest work; refresh queued ACPX deadline; back off ACPX sterile retries; reconcile failed vector writes; cache routing config loads; bound imported documents card.
